...
def add_favorite_planet(planet_id):
    ...
    new_favorite_planet = Favorite_location(
        user_id=data.get("user_id"), location_id=planet_id)
...
def add_favorite_people(people_id):
    ...
    new_favorite_character = Favorite_character(
        user_id=data.get("user_id"), character_id=people_id)
...
def delete_favorite_planet(planet_id, user_id):
    favorite_planet = db.session.execute(select(Favorite_location).where(
        Favorite_location.user_id == user_id, Favorite_location.location_id == planet_id)).scalar_one_or_none()
    if favorite_planet is None:
        return jsonify({"error": "no tienes planetas favoritos"}), 404
...
def delete_favorite_people(people_id, user_id):
   favorite_people = db.session.execute(select(Favorite_character).where(
       Favorite_character.user_id == user_id, Favorite_character.character_id == people_id)).scalar_one_or_none()
   if favorite_people is None:
        return jsonify({"error": "no tienes personajes favoritos"}), 404
